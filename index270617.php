...
function my_strtoupper($str){  
	...
	//$str=strtoupper($str);	// for Windows-1251 
	return $str;
...
if($language=="en"){$langstr="/en";$unlangstr="";$tovarstr="A new item has been added to your Shopping Cart.";
	...
	$showallstr="Показывать всё";$shownotallstr="Только с фотографией";$logogif="/logo_ifarfor.gif";$MainLabel="Императорский фарфор. Официальный магазин ИФЗ.";
...
/*
if($sortby=="cost") {$sortpage=$sortstr." <a href='".aPSID("$stroka_sort/sortbyname$firstpage$numberofpages$ShAll")."'>$sortstr2 </a> $ifstr <b>$sortstr3</b>";$Sort='p';}
else 			    {$sortpage=$sortstr." <b>$sortstr2</b> $ifstr <a href='".aPSID("$stroka_sort/sortbycost$firstpage$numberofpages$ShAll")."'>$sortstr3</a>";$Sort='n';}
if($ShAll=="1") {$Showpage="<b>$showallstr</b> $ifstr <a href='".aPSID("$stroka_sort/sortby$sortby"."01120")."'>$shownotallstr</a>";}
else 			{$Showpage="<a href='".aPSID("$stroka_sort/sortby$sortby"."01121")."'>$showallstr</a> $ifstr <b>$shownotallstr</b>";}
*/
if($menuname==''){$menuname='shop';$menuname2='ware';global $HotStr3; 
	$HotStr3='newyear';$HotStr3='cobaltnet';$HotStr3='love';;$HotStr3='easter';$HotStr3='love';$HotStr3='russianstyle';}////$HotStr3='cobaltnet';$menuname='shop';$menuname2='ware';$menuname3='teacof';
//	if($menuname=='') {$menuname='/index.php?search=tag%CD%CE%C2%DB%C9%20%C3%CE%C4';}
//$menuname4='teaserv';
///if($menuname2=='' and $menuname=='shop') {$menuname2='ware';$menuname3='teacof';}
///if($menuname3=='' and $menuname=='shop' and $menuname2=='stoppard') $menuname3='plate200';
///if($menuname4=='' and (substr($menuname3, 0, 4)=="sort") and $menuname=='shop' and $menuname2=='stoppard') {$menuname4=$menuname3; $menuname3='plate200';}
///if($menuname3=='' and $menuname=='shop' and $menuname2=='tableware') $menuname3='settw';
$menu=array(
	...
	array("stoppard","Декоративные тарелки","plate150","Декоративные тарелки 150мм","plate200","Декоративные тарелки 200мм","plate270","Декоративные тарелки 270мм","decoplate","Декоративные тарелки ИФЗ","holder","Подставки для тарелок"),//"vodka","Для водки","water","Для воды","martini","Для мартини","champagne","Для шампанского","glasses","Стеклянная посуда"
...
$menuenglish=array(
	...
	array("stoppard","Decorative plates","plate150","Decorative plates 150mm","plate200","Decorative plates 200mm","plate270","Decorative plates 270mm","decoplate","Decorative plates IPM","holder","Holder for plates"),//"vodka","Для водки","water","Для воды","martini","Для мартини","champagne","Для шампанского","glasses","Стеклянная посуда"
...
//clearstatcache();
//Заголовок
//Ищем есть ли в строке товар для Фильтрации
/*$Filter='';//$sortway='';$sortstr='';
if        (substr($menuname2, 0, 3)=="fil"){$Filter=substr($menuname2, 3);$menuname2="";}
elseif(substr($menuname3, 0, 3)=="fil"){$Filter=substr($menuname3, 3);$menuname3="";}
elseif(substr($menuname4, 0, 3)=="fil"){$Filter=substr($menuname4, 3);$menuname4="";}
elseif(substr($menuname5, 0, 3)=="fil"){$Filter=substr($menuname5, 3);$menuname5="";}
elseif(substr($menuname6, 0, 3)=="fil"){$Filter=substr($menuname6, 3);$menuname6="";};
if($Filter!='') {
$Filter=urldecode($Filter);
$filterstr='f_'.urlencode($Filter);};//  | - это разделитель слов для запроса на фильтрацию*/
...
$sortby='name';$sortway='';$sortstr='';$firstpage="01";$numberofpages="21";$ShAll="0";
...
else			$sortstr='sortbyname0121'.$ShAll;//по умолчанию сортируем по имени
...
if($menuname3!=""){
	$ruru=sql("SELECT * FROM tovsNew WHERE id='$menuname3' and grp='1'");$countruru=mysql_num_rows($ruru);$ji=0;
	if($countruru>0){
		$rowjuk=mysql_fetch_array($ruru);
	...
	mysql_free_result($ruru);
}	
if($menuname4!=""){
	$ruru=sql("SELECT * FROM tovsNew WHERE id='$menuname4' and grp='1'");$countruru=mysql_num_rows($ruru);$ji=0;
	if($countruru>0){
		$rowjuk=mysql_fetch_array($ruru);
		if($language=='en') $juk=$rowjuk[english];
		else $juk=$rowjuk[name];
...
if($menuname5!=""){
	$ruru=sql("SELECT * FROM tovsNew WHERE id='$menuname5' and grp='1'");$countruru=mysql_num_rows($ruru);$ji=0;
	if($countruru>0){
		$rowjuk=mysql_fetch_array($ruru);
		if($language=='en') $juk=$rowjuk[english];
		else $juk=$rowjuk[name];
...
if($ShAll=="1"){$Showpage="<b>$showallstr</b> $ifstr <a href='".aPSID("$stroka_unfull/sortby$sortby"."01120")."'>$shownotallstr</a>";}
else{$Showpage="<a href='".aPSID("$stroka_unfull/sortby$sortby"."01121")."'>$showallstr</a> $ifstr <b>$shownotallstr</b>";}
/*
if($sortby=="cost") {$sortpage="Сортировать по: <a href='".aPSID("$stroka_sort/sortbyname$firstpage$numberofpages$ShAll")."'>Названию </a> или <b>Цене</b>";$Sort='p';}
else 			    {$sortpage="Сортировать по: <b>Названию</b> или <a href='".aPSID("$stroka_sort/sortbycost$firstpage$numberofpages$ShAll")."'>Цене</a>";$Sort='n';}
if($ShAll=="1") {$Showpage="<b>Показывать всё</b> или <a href='".aPSID("$stroka_sort/sortby$sortby"."01120")."'>Только с фотографией</a>";}
else 			{$Showpage="<a href='".aPSID("$stroka_sort/sortby$sortby"."01121")."'>Показывать всё</a> или <b>Только с фотографией</b>";}
*/
...
for($i=0;$i<7;$i++){
	...
	if($menuLink=="stoppard") $menuLink="stoppard/plate200/stillife";
...
//модуль для корзины
//echo '<!-- jQuery Reveal --><script type="text/javascript" src="/jquery/reveal/jquery.reveal.js"></script>
//<link rel="stylesheet" type="text/css" href="/jquery/reveal/reveal.css" />';
/* Reveal CSS */
//echo '<link rel="stylesheet" href="/jquery/reveal/reveal.css">';
/* jQuery */
echo '	<script type="text/javascript" src="/jquery-1.8.2.min.js"></script>';
echo"<script>
function LikeEngine(tag){
var html = $.ajax({
url: 'index2017.php?id='+ tag+'&uid=".$userid."',
async: false
}).responseText;
...
/*
var str='index2017.php?id=' + tag;
$.get( 'index2017.php', function( data ) 
{
str+=data;
...
<a href="'.aPSID("$langstr/").'" target="_self">
...
{$stroka='';
	...
	if(mysql_num_rows($query1)>0){
		...
		if($language=="en"){
			$roww=mysql_fetch_array(sql("SELECT * FROM picture WHERE id='".$row['Picture']."'"));$rowPictureR=$roww['name'];$rowPicture=$roww['english'];if($rowPicture=="") $rowPicture=$roww['name'];
			$roww=mysql_fetch_array(sql("SELECT * FROM form WHERE id='".$row['Form']."'"));$rowFormR=$roww['name'];$rowForm=$roww['english'];if($rowForm=="") $rowForm=$roww['name'];
			$roww=mysql_fetch_array(sql("SELECT * FROM material WHERE id='".$row['TipOfMaterial']."'"));$rowTipOfMaterial=$roww['english'];if($rowTipOfMaterial=="") $rowTipOfMaterial=$roww['name'];
			$roww=mysql_fetch_array(sql("SELECT * FROM creator WHERE id='".$row['AutorForm']."'"));		
			$rowAutorForm=$roww['english'];
			$roww=mysql_fetch_array(sql("SELECT * FROM creator WHERE id='".$row['AutorPicture']."'"));	
			$rowAutorPicture=$roww['english'];
			$roww=mysql_fetch_array(sql("SELECT * FROM tovsNew LEFT JOIN brand ON brand.id = tovsNew.Factory WHERE tovsNew.ida='$view'"));			
			...
			$roww=mysql_fetch_array(sql("SELECT name FROM picture WHERE id='".$row['Picture']."'"));
			$rowPicture=$roww['name'];$rowPictureR=$rowPicture;
			$roww=mysql_fetch_array(sql("SELECT name FROM form WHERE id='".$row['Form']."'"));
			$rowForm=$roww['name'];$rowFormR=$rowForm;
			$roww=mysql_fetch_array(sql("SELECT name FROM material WHERE id='".$row['TipOfMaterial']."'"));		
			$rowTipOfMaterial=$roww['name'];
			$roww=mysql_fetch_array(sql("SELECT name FROM creator WHERE id='".$row['AutorForm']."'"));		
			$rowAutorForm=$roww['name'];
			$roww=mysql_fetch_array(sql("SELECT name FROM creator WHERE id='".$row['AutorPicture']."'"));	
		...
		$frontname=MakeFrontName($rowname,$rowVid,$rowTip,$rowPicture,$rowForm,$rowTipOfMaterial,$rowPerson,$rowPredmetov,
		...
		if($rowVid!='Сервиз'){
			...
			if($rowmemberID1!=''){
				...
				$countruru=mysql_num_rows($ruru);
			}
			if($countruru>0 and $rowmemberID1!=''){
				$rowmem = mysql_fetch_array($ruru);
				$rowname=$rowmem['name'];
				$rowVid=$rowmem['Vid'];
				$rowTip=$rowmem['Tip'];
				...
				if($rowmemberID2!=''){
					...
					$rowmem = mysql_fetch_array(sql("SELECT * FROM tovsNew WHERE id='$rowmemberID2'"));
					$rowname=$rowmem['name'];
					$rowVid=$rowmem['Vid'];
					$rowTip=$rowmem['Tip'];
				...
				if($rowmemberID3!=''){
					...
					$rowmem = mysql_fetch_array(sql("SELECT * FROM tovsNew WHERE id='$rowmemberID3'"));
					$rowname=$rowmem['name'];
				...
				if($rowmemberID4!=''){
					...
					$rowmem = mysql_fetch_array(sql("SELECT * FROM tovsNew WHERE id='$rowmemberID4'"));
					$rowname=$rowmem['name'];
					$rowVid=$rowmem['Vid'];
					$rowTip=$rowmem['Tip'];
		...
		if(mysql_num_rows($query1)>0){
			$row = mysql_fetch_array($query1);
			$rowVid=$row['Vid'];
			$roww=mysql_fetch_array(sql("SELECT * FROM picture WHERE id='".$row['Picture']."'"));$rowPictureR=$roww['name'];
			$roww=mysql_fetch_array(sql("SELECT * FROM form WHERE id='".$row['Form']."'"));$rowFormR=$roww['name'];
	...
	echo "</td>";
	echo '</tr></table>';
	mysql_free_result($ruru);	mysql_free_result($query1);
}
elseif(($menuname=="shop" ) and $view==''){//or $searchline!=""
	...
	for($i=2;$i<count($menu[$m2]);$i=$i+2){
		...
		while($rowruru2=mysql_fetch_array($ruru2)){
	...
	if($HotStr!='zu' and $HotStr3==''){
		...
		';
		mysql_free_result($ruru2);	mysql_free_result($ruru5);
		//=============================================================================================
		...
		if(mysql_num_rows($rurus)>0) while($roww=mysql_fetch_array($rurus)) if($roww[$nameof]!='')$a+=array($roww[$nameof]=> $roww['id']);	ksort ($a);
		$aform = array();
		if($menuname4==''){
			if($menuname3==''){
				if($menuname2==''){$a='';}
		...
		if(mysql_num_rows($rurus)>0) while($roww=mysql_fetch_array($rurus)) if($roww[$nameof]!='')$aform+=array($roww[$nameof]=> $roww['id']); ksort ($aform);
		$amat = array();
		if($menuname4==''){
			if($menuname3==''){
				if($menuname2==''){$amat='';}
		...
		//========================Если выбор не пустой, то формируем список Форм и добавляем их в массив $a=======
		if(mysql_num_rows($rurus)>0) while($roww=mysql_fetch_array($rurus)) if($roww[$nameof]!='')$amat+=array($roww[$nameof]=> $roww['id']); ksort ($amat);
		$abr = array();
		if($menuname4==''){
			if($menuname3==''){
				if($menuname2==''){$abr='';}
		...
		if(mysql_num_rows($rurus)>0) while($roww=mysql_fetch_array($rurus)) if($roww[$nameof]!='')$abr+=array($roww[$nameof]=> $roww['id']); 
		ksort ($abr);
		mysql_free_result($rurus);
	...
	if($language=="en"){
		...
		echo'</div><div class="hot">';if($HotStr=='nega')echo'<b>НЕГА</b>'; else 
		echo'<a href="'.aPSID("/index.php?search=tagnega").'" class="link1">НЕГА</a>'; 
...
elseif($menuname=="cabinet"){
	...
	if($menuname2=="create") echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/create.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="worked")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/worked.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="archive")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/archive.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="signin")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/signin.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="adminwork")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="YES" width="100%" height="1000px;" id=order name=order src="'.aPSID('/adminworking.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="reset")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/reset.php?uid='.$menuname3.'&sw='.$menuname4.$resettext.$langreset).'"></IFRAME>';
	elseif($menuname2=="logout")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/logout.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="pay")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/pay.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="edit")	echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/cabinet.php'.$langaddstr).'"></IFRAME>';
	elseif($menuname2=="order")echo '<IFRAME hspace="0"  frameborder="0" marginheight="0" marginwidth="0" vspace="0" scrolling="No" width="100%" height="1000px;" id=order name=order src="'.aPSID('/order.php'.$langaddstr).'"></IFRAME>';
...
function MakeStrokeSQL($HotStr2,$page,$group,$Uslovie,$Filter,$Sort,$ShAll){
	...
	$rurusi=sql("SELECT * FROM form"); $countrurus=mysql_num_rows($rurusi);
	if($countrurus>0) while($roww=mysql_fetch_array($rurusi)) if($roww['name']!='')$a+=array($roww['name']=> $roww['id']);	
	ksort ($a);
	foreach($a as $k => $t){
	...
	mysql_free_result($rurusi);
	//==============вставка============================================================================
	//==============вставка=================================по производителю отбор====================
	$a = array();$RightUslovie2='';
	$rurusi=sql("SELECT * FROM brand"); $countrurus=mysql_num_rows($rurusi);
	if($countrurus>0) while($roww=mysql_fetch_array($rurusi)) if($roww['name']!='')$a+=array($roww['name']=> $roww['id']);	
	ksort ($a);
	foreach($a as $k => $t){
	...
	$countquery1=mysql_num_rows($query0);
...
function MakeStrokeSQLMat($HotStr,$page,$group,$Uslovie,$Filter,$Sort,$ShAll){
	...
	$a = array();$RightUslovie2='';
	$rurusi=sql("SELECT * FROM picture"); $countrurus=mysql_num_rows($rurusi);
	if($countrurus>0) while($roww=mysql_fetch_array($rurusi)) if($roww['name']!='')$a+=array($roww['name']=> $roww['id']);	
	ksort ($a);
	foreach($a as $k => $t){
		if(isset($_POST["pcx$t"]) and   $_POST["pcx$t"] == 'yes'){
...
function PrintCatalog($HotStr,$page,$group,$Uslovie,$Filter,$Sort,$RightUslovie,$stroka_sort,$firstpage,$numberofpages,$ShAll,$language){
	...
	$countquery1=mysql_num_rows($query0);
	while($row = mysql_fetch_array($query0)){$iddd=$row['id'];$SQLZapros=$SQLZapros." or idg ='$iddd'";};
	if($ShAll==1)  $SQLZapros.=")AND grp=0 AND ((zakaz+sklad+grp)>0) $Uslovie";//post('ShowFoto')=='yes' or 
	else //Если товаров с фотографиями в группе нет - покажем ка мы даже без фото! 
	{
		...
		$countquery1=mysql_num_rows($query1);
	...
	if($HotStr3!=''){
		...
		if($HotStr3=="newyear"){$HotStr="SELECT name,ida,id,idg,price1s,sklad,vid,Tip,picture,AutorPicture,form,TipOfMaterial,factory,Imagefile, Person, Predmetov FROM tovsNew WHERE NY='1' $SQLZapros";}
		elseif($HotStr3=="cobaltnet"){$HotStr="SELECT  tovsNew.Height,tovsNew.Capacity,tovsNew.Width,tovsNew.Diameter,tovsNew.name,ida, tovsNew.id,price1s,sklad,vid,Tip,picture,AutorPicture,form,TipOfMaterial,factory,Imagefile, Person, Predmetov FROM picture LEFT JOIN tovsNew ON picture.id = tovsNew.Picture WHERE picture.name='Кобальтовая сетка' $SQLZapros";}
		elseif($HotStr3=="love"){$HotStr="SELECT * FROM tovsNew WHERE InLove='1' $SQLZapros";}
		elseif($HotStr3=="easter"){$HotStr="SELECT videnglish,tipenglish,name,ida,id,idg,price1s,sklad,vid,Tip,picture,AutorPicture,form,TipOfMaterial,factory,Imagefile, Person, Predmetov FROM tovsNew WHERE Easter='1' $SQLZapros";}
	...
	mysql_free_result($query1);mysql_free_result($query0);
	return $echo;
}
//========================================================================================================
//========================================================================================================
...
function printsearch($searchline){
	...
	elseif(substr($searchline, 0, 8)=="tagaform"){
		...
		$sql_filter.='%"';
		$Uslovie.=$sql_filter." AND sklad>0 AND Imagefile<>'/icons/noimage.jpg' ";
		$HotStr="SELECT tovsNew.name,ida,idg, tovsNew.id,price1s,sklad,vid,Tip,picture,AutorPicture,form,TipOfMaterial,factory,Imagefile, Person, Predmetov FROM creator LEFT JOIN tovsNew ON creator.id = tovsNew.AutorForm  WHERE $Uslovie ORDER BY tovsNew.name";//
		$r=sql($HotStr);
	}
	elseif(substr($searchline, 0, 3)=="tag"){
		...
		elseif($HotStr=="cobaltnet"){
			$r=sql("SELECT id FROM picture WHERE name='Кобальтовая сетка'");$row=mysql_fetch_array($r);$id1=$row[id];
			$HotStr="SELECT * FROM tovsNew WHERE Picture='$id1' $SQLZapros";}
		elseif($HotStr=="nega"){
			$r=sql("SELECT id FROM form WHERE name='Нега'");$row=mysql_fetch_array($r);$id1=$row[id];
			$HotStr="SELECT * FROM tovsNew WHERE Form='$id1' $SQLZapros";}
		elseif($HotStr=="zamoscow"){
			$r=sql("SELECT id FROM picture WHERE name='Замоскворечье'");$row=mysql_fetch_array($r);$id1=$row[id];
			$HotStr="SELECT * FROM tovsNew WHERE Picture='$id1' $SQLZapros";}
		//elseif($HotStr=="newyear"){$HotStr="SELECT  tovsNew.Height,tovsNew.Capacity,tovsNew.Width,tovsNew.Diameter,tovsNew.name,ida, tovsNew.id,price1s,sklad,vid,Tip,picture,AutorPicture,form,TipOfMaterial,factory,Imagefile, Person, Predmetov FROM picture LEFT JOIN tovsNew ON picture.id = tovsNew.Picture WHERE picture.name='Замоскворечье' $SQLZapros";}
		elseif($HotStr=="nephrit"){
			$r=sql("SELECT id FROM picture WHERE name='Нефритовый фон'");$row=mysql_fetch_array($r);$id1=$row[id];
			$HotStr="SELECT * FROM tovsNew WHERE Picture='$id1' $SQLZapros";}
		...
		if($searchline!=""){
			...
			$qq=explode(" ",$searchline); 
			$num = count($qq);
			for($i = 0; $i < $num; $i ++ ) $sql_filter.=' AND ( lower(name) LIKE lower ("%'.($qq[$i]).'%") )';
			$sql_filter.=') OR (grp=0';
			//$Uslovie.=$sql_filter;
			//english
			//$qq=explode(" ",$searchline); 
			//$num = count($qq);
			for($i = 0; $i < $num; $i ++ ) $sql_filter.=' AND (lower(english) LIKE "%'.strtolower(($qq[$i])).'%")';
			$sql_filter.=')';
			$Uslovie.=$sql_filter;
		}
	...
	if(mysql_num_rows($r)==0){$echo="<tr><td style='padding-top:20px;' colspan='5'>$strnothing1 $searchline $strnothing2 $query</td></tr>"; 
		$Zagolovok='';}
	else{
		...
		$countquery1=mysql_num_rows($r);
		$i=1;$j=1;
		while($row = mysql_fetch_array($r)){
			$idg=$row['idg'];
			$queryz=sql("SELECT * FROM tovsNew WHERE id='$idg' AND grp=1 ");
			$countqueryz=mysql_num_rows($queryz);
			if($countqueryz>0){
				...
				$echo.='<div id="like'.$id.'" onClick="LikeEngine(\'like'.$id.'\')" style="font-size:14px;font-weight:500;color:#AD9E82;vertical-align:top;">
				<span style="display: block; float: left; margin: 10px 0 0 0; text-align: center; vertical-align:middle;"><img src="img/heart3.gif"></span>
				<span style="display: block; float: left; margin: 13px 0 10 0; padding-left:5px;text-align: center; vertical-align:middle;">Нравится</span></div>';
				$echo.="</td></tr></table></td>";
	...
	$echo.='<div id="scrollup"><img width="50px" alt="Прокрутить вверх";" src="/img/up.png"></div></td>';
	mysql_free_result($queryz);mysql_free_result($r);

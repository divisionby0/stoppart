...
class StrokeSQLRequestBuilder
{
    ...
    public function createSimple($HotStr2,$page,$group,$Uslovie,$Filter,$Sort,$ShAll){
        ...
        $a = array();
        $RightFormUslovie2='';
        $rurusi=sql("SELECT * FROM form"); 
        $countrurus=mysqli_num_rows($rurusi);
    ...
    public function createMat($HotStr,$page,$group,$Uslovie,$Filter,$Sort,$ShAll){
        ...
        $a = array();
        $RightUslovie2='';
        $rurusi=sql("SELECT * FROM picture"); 
        $countrurus=mysqli_num_rows($rurusi);
        if($countrurus>0) while($roww=mysqli_fetch_array($rurusi)) if($roww['name']!='')$a+=array($roww['name']=> $roww['id']);
        ksort ($a);
        foreach($a as $k => $t){
        ...
        if($ShAll=="1") {
            $SQLZapros.=")AND grp=0 AND ((zakaz+sklad+grp)>0) $Uslovie";
        }
        else {
            $SQLZapros.=")AND grp=0 AND Imagefile<>'/icons/noimage.jpg' AND ((zakaz+sklad+grp)>0) $Uslovie";
        }

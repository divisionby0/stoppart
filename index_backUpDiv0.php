...
$menuenglish=array(
	...
	array("stoppard","Decorative plates","plate150","Decorative plates 150mm","plate200","Decorative plates 200mm","plate260","Decorative plates 260mm","decoplate","Decorative plates IPM","holder","Holder for plates"),
...
/*echo"<script>function SizeEngine(tag,size){
var html = $.ajax({url: '/le.php?id=' +tag+'&size='+size,async: false}).responseText;
document.getElementById(tag).innerHTML=html;}</script>";*/
/*
	...
	while($row = mysqli_fetch_array($query1)){
		...
		if($counterofpage>$firstpiece and $counterofpage<=$lastpiece){
			...
			if($i<=3) $sti="padding-right:10px;";else $sti='';$Prov=ProverkaNal($id,'5');
				...
				if($brandid==16)$sizelike='
				<div>
				<input data-itemId="'.$ida.'" name="optionRadio'.$id.'" id="size1'.$id.'" value="150" type="radio"> 150мм
				<input data-itemId="'.$ida.'" name="optionRadio'.$id.'" id="size2'.$id.'" value="200" type="radio"> 200мм 
				<input data-itemId="'.$ida.'" name="optionRadio'.$id.'" id="size3'.$id.'" value="260" type="radio"> 260мм </div>
				';
				//if(isset($userid))	
			...
			if(($Prov=='0') and ($pobeda=='1')){$i=$i-1;echo'1';} 
			else $echo.="<td class='ifzshop' data-itemId='".$ida."' style='$sti'>
			...
			<a href='".aPSID("/set.php?oper=add_tov&tov_id=$id&language=$language")."' id='ishop$id' target='market' id='ishop' >$strbask </a>$userid$printlike$sizelike
			</td></tr></table></td>";
